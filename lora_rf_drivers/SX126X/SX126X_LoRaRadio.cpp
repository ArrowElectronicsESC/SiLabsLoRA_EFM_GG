...
extern void Delay(uint32_t dlymsTicks);
...
SX126X_LoRaRadio::SX126X_LoRaRadio(unsigned int mosi,
		                           unsigned int miso,
		                           unsigned int sclk,
		                           unsigned int nss,
		                           unsigned int reset,
		                           ...
		                           unsigned int device_select,
		                           unsigned int crystal_select,
		                           unsigned int ant_switch)

    : _spi(mosi, miso, sclk),
      _chip_select(gpioPortE,nss,gpioModeInput, 1),
      _reset_ctl(gpioPortB,reset,gpioModeInput, 1),
      _dio1_ctl(gpioPortA,dio1,gpioModeInput, 0),
      _busy(gpioPortA,busy,gpioModeInput, 0 ),
      _freq_select(gpioPortE,freq_select,gpioModeInput, 0),
      _dev_select(gpioPortE,device_select,gpioModeInput, 0),
      _crystal_select(gpioPortC,crystal_select, gpioModeInput, 0),
      _ant_switch(gpioPortB,ant_switch, gpioModeInput, 1)
{
    _radio_events = NULL;
    _reset_ctl = 1;
    ...
    RTOS_ERR  err;
    OSMutexCreate(&taskmutex, "task mutex", &err);
...
void SX126X_LoRaRadio::lock(void)
{
	...
	CPU_TS ts;
	OSMutexPend(&taskmutex, 0, OS_OPT_PEND_BLOCKING, &ts, &err);
	APP_RTOS_ASSERT_DBG((RTOS_ERR_CODE_GET(err) == RTOS_ERR_NONE), 1);
}
...
void SX126X_LoRaRadio::unlock(void)
{
	RTOS_ERR  err;
	CPU_TS ts;
	OSMutexPost(&taskmutex, OS_OPT_POST_NONE, &err);
	APP_RTOS_ASSERT_DBG((RTOS_ERR_CODE_GET(err) == RTOS_ERR_NONE), 1);
}
                                             ...
                                             uint32_t max_carrier_sense_time)
{
    ...
    uint32_t curTicks,startTicks;
    startTicks = readmsTicks();


    // Perform carrier sense for maxCarrierSenseTime
    while (curTicks - startTicks < (int) max_carrier_sense_time) {
    	curTicks = readmsTicks();
        rssi = get_rssi();
...
void SX126X_LoRaRadio::init_radio(radio_events_t *events)
{
    ...
    GPIO_IntConfig(_dio1_ctl._port, _dio1_ctl._pin, true, false, true);
...
{
    uint16_t val = 0;
    val = MBED_CONF_LORA_DEVICE_SELECT;
...
{
    uint16_t val = 0;
    val = MBED_CONF_LORA_FREQ_SELECT;
