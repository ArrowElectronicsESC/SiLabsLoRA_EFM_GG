...
#include "platform/DigitalInOut.h"
#include "platform/DigitalIn.h"
...
public:
    SX126X_LoRaRadio(unsigned int mosi,
    		unsigned int miso,
			unsigned int sclk,
			unsigned int nss,
			unsigned int reset,
...
private:
    ...
    // Interrupt controls
    mbed::DigitalIn _dio1_ctl;;

    // module busy control
    mbed::DigitalIn _busy;

    // module frequency selection
    mbed::DigitalIn _freq_select;

    // module device variant selection
    mbed::DigitalIn _dev_select;
